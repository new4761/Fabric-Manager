...
class SpecConfig extends FileYamlBuilder implements YamlConfig {
    ...
    orderers: string[] = [];
    src:string="";
    defaultOrderer: OrgData;
    constructor() {
        super();
        this.fileName = "spec.yaml"
        this.defaultOutputPath = "bin";
        this.defaultOrderer = this.newOrg("orgDefault", true);
        this.orderers.push("orderer" + this.orderers.length + "." +this.defaultOrderer.name);
    }
    createFile() {
    ...
    let minifabFileName ="minifab.cmd"
        ...
        let minifabPath = path.join(process.cwd(),"src","assets","minifab",minifabFileName)
    ...
    switchOrgRole(target: OrgData) {
        ...
        else { target.setToOrderer() }
    }

    addOrdererOrg() {
        this.orgList.forEach(item => {
            if (item.isOrderer == true && item.name != "orgDefault") {
                this.orderers.push("orderer" + this.orderers.length + "." + item.name);
            }
        })
    }
    resetData() {
        this.orgList = [];
        this.orgList.push(this.defaultOrderer);
        this.CAList = [];
        this.peerList = [];
        this.orderers = [];
        this.orderers.push("orderer" + this.orderers.length + "." + this.defaultOrderer.name);
    ...
    testAdd() {
        //  console.log("testADD CALLED");
        let org1 = this.newOrg("org1", false);
        let org2 = this.newOrg("org2", false);
        let Orderer = this.newOrg("ufo", true);
        org1.addCA();
        org1.addCA();
        org1.addPeer();
        org1.addPeer();
